...
display through applying different methods, and save the altered image. These altercations do not
actually modify the original image but rather save them as separate files. The supported file
...
which is able to extract out individual RGB values, brighten, and greyscale. A method for
...
former, (255,255,255) would represent white, while for the latter, (1,1,1) would represent white.

Image is an interface that represents an image. An image is composed of pixels. Therefore,
the ImageImpl class, which implements the Image interface, has a field of IPixel in a 2D array format.
A class implementing Image is able to get its width, height, maximum RGB value, pixels 2D array,
and a specific pixel. All of these do not return the actual field, since the fields must be
...
This interface is built apart from the ImageProcessorModel interface, in order to separate out the
...
FlipType. However, it now contains four enumerations: GreyscaleType, FlipType,
...
the idea of Player enumeration back from the Marble Solitaire project. FilteringType consists of two
...
also contains a method to get an image out from the image storage. The image storage format is under
the map. This is because using a map allows us to easily search for the image just with its key,
...
A new interface called MaskProcessorModel has been added to support the ability of
...
interface. In this class, there are four methods that support image manipulations: brighten,
...
ASCII PPM file. Private helper functions, loadOther and saveOther, have been added. These
...
format, we created other private helper functions called loadPPM and savePPM. The idea was
inspired by the provided ImageUtil file. Also, there were some design changes
...
added to transmit a message to the view. Downscale and mask features have been added. As
a result, the code for the runProcessor method has been newly implemented. The inputs for downscale are
...
Features is another interface that has been added. This is a controller interface specifically
...
A new method for downscale has been added. The mask feature has also been added to every
method to support the ability to apply any of the existing image manipulations to only part of an
image.

A new package called util has also been added.
...
An implementation of the view has been added.

## View

View contains two interfaces: ImageProcessorView and ImageProcessorGUIView.

ImageProcessorView is an interface allowing the user to view outputs produced by an image processor.
...
screen anytime it needs to be doing so. This is called when something on the screen is updated
...
produces four different overlapped graphs: R, G, B, and Intensity. The constructor of the
...
A condition has been added. Now, there are two ways to use the main class. If its
configuration takes in an empty program, just like how it used to work before, it will take in
System.in as an input. However, a new feature allows one to take in a script.txt file. The second
condition is made so that the script file can run on a jar file. If the configuration takes in
'-file res/script.txt' as a program argument, and the main class is run, the file reader would read
the script-txt file inside the res folder and use that as the readable input. Because the script file
is specifically built for the res folder, in actuality, house.ppm wouldn't be loaded. However,
the purpose of the second condition is exclusively for jar files, so that's appropriate.

There has been a small modification in the code to add the mask feature when running the
